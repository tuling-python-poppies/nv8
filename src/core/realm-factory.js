 ...
 * 执行时序被改变：`await import('../presets/index.js')` 加上读 28 个插件
...
const PAGE_PARSER_URL = new URL(
  '../api/dom/html-parser.js',
  import.meta.url,
);
const CORE_NAVIGATION_URL = new URL(
  '../install/install-core-navigation.js',
  import.meta.url,
);
...
const WINDOW_CONTEXT_URL = new URL(
  '../install/install-window-context.js',
  import.meta.url,
);
const SERVICE_WORKER_RUNTIME_URL = new URL(
  '../api/worker/service-worker-runtime.js',
  import.meta.url,
);
const WORKER_GLOBAL_RUNTIME_URL = new URL(
  '../api/worker/worker-global-runtime.js',
  import.meta.url,
);
