 ...
 */
export function createPluginInstallError(pluginId, phase, originalError, details = {}) {
  const message = `Plugin '${pluginId}' failed during ${phase}: ${originalError.message}`;
 ...
 */
export function createDependencyError(code, message, context) {
  return createDiagnosticError(code, message, {
    phase: 'resolve',
    context,
    suggestions: [
      'Check plugin dependencies and versions',
      'Ensure all required plugins are registered',
    ],
  });
}

/**
 * Surface collision error
 */
export function createSurfaceCollisionError(target, property, existingOwner, newOwner) {
  return createDiagnosticError(
    ErrorCode.SURFACE_COLLISION,
    `Surface collision: ${target}.${property} is already owned by '${existingOwner}'`,
    {
      phase: 'install',
      context: {
        target,
      ...
      suggestions: [
        ...
        'Use Profile pins to select a specific provider',
        'Contact plugin authors to resolve naming conflicts',
      ],
    }
  );
}
