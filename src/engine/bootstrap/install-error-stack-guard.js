...
// `src/` 根，用于把 NV8 自己的帧从堆栈里过滤掉。
//
// **必须是纯字符串操作，不能用 `new URL()`**：这个模块在 Realm 内求值，而此刻
// `hideNodeGlobals()` 已经删掉 Node 的 `URL`、`installURL()` 还没跑。改成
// `new URL("../../", import.meta.url)` 会在引导阶段抛 `URL is not defined`
// ——目录分层时踩过一次。
//
// 原写法是 `indexOf("/bootstrap/") + 1`，依赖「本文件的父目录就是 src」这个巧合，
// 一分层就会截到 `src/engine/`，`src/surface/` 下的内部帧于是不再被识别。
// 现在按 `/src/` 定位，与目录层数无关。
