...
/**
 * 校验 Profile Lock Plan
 * 
 * @param {ProfileLockPlan} lockPlan - Lock plan
 * @throws {Error} 校验失败
 */
export function validateProfileLockPlan(lockPlan) {
  if (!lockPlan || typeof lockPlan !== 'object') {
    throw new Error('Profile lock plan must be an object');
  }
  
  if (!lockPlan.profileId || typeof lockPlan.profileId !== 'string') {
    throw new Error('Profile lock plan must have a valid "profileId"');
  }
  
  if (!lockPlan.profileVersion || typeof lockPlan.profileVersion !== 'string') {
    throw new Error('Profile lock plan must have a valid "profileVersion"');
  }
  
  if (!lockPlan.createdAt || typeof lockPlan.createdAt !== 'string') {
    throw new Error('Profile lock plan must have a valid "createdAt" timestamp');
  }
  
  if (!Array.isArray(lockPlan.plugins)) {
    throw new Error('Profile lock plan "plugins" must be an array');
  }
