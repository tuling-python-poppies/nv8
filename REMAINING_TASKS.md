...
- **测试状态**: 1002 项（`npm test`，110 个文件）。Node 18 / 20 / 22 / 24
  四档全绿
- **项目性质**: 私有框架，无公开发布计划
...
- [x] 所有后端异常的句柄泄漏验证
  ...
  - [x] SharedWorker 图创建失败：错误事件仍派发，live graph、child Realm 和两端 MessagePort 均释放
    （`tests/resource-lifecycle-test.js`，child-process / worker-thread）
  - [x] ServiceWorker 注册失败：registration、child Realm 和 ServiceWorker 资源计数均释放
    （`tests/resource-lifecycle-test.js`，child-process / worker-thread）
...
- [x] **被顶掉的导航仍会建出子 Realm 再关掉** - 非空白 iframe 导航在微任务中检查最新版本与连接状态，被覆盖或移除的导航不会创建无效子 Realm
...
- [x] **API 文档** - `docs/api-reference.md` 覆盖 package root、Nv8Instance、Profile/preset、Protocol、Collector、
  生命周期、错误处理和安全边界；`tests/docs-contract-test.js` 校验公开入口与关键约束
- [x] **示例代码** - `examples/` 提供最小 Realm 求值、Artifact→Protocol→Collector 链路、
  cursor 分页与结果去重三个可运行示例；`tests/examples-test.js` 以子进程实际执行并验证输出
...
- [x] Core SemVer 策略（私有项目可简化） - 独立维护 Core `major.minor.patch` 版本，
  Lock Plan 记录 `coreVersion` 并纳入 digest；提供 `*`、精确、`^`、`~` 和比较范围校验
- [x] Plugin lock 格式和签名（可选） - `PLUGIN_LOCK_SCHEMA = nv8.plugin-lock/v1`；新增 Ed25519
  签名/验证，签名覆盖去掉 `signature` 后的 canonical Lock Plan；信任公钥由调用方按 `keyId`
  提供，验证端拒绝私钥并对篡改/未知算法 fail closed
- [x] Evidence Bundle canonical JSON - manifest 签名覆盖移除 `signature` 后的确定性 canonical JSON
- [x] Frame Protocol 表示格式 - `docs/frame-protocol.md` 固化 16 字节 big-endian 头部、typed value
  payload、分片/多帧读取、版本拒绝、长度限制和协议错误后的连接终止规则；`tests/frame-protocol-test.js`
  覆盖跨 chunk 边界与非法帧
...
- [x] Protocol schema 和版本策略 - 新增 Protocol 层独立 `schemaVersion`（当前 `1.0`），registry lock
  与 ProtocolResult 均显式输出；同主版本旧 minor 可消费，未来 minor/不同 major 返回
...
- [x] **换基准到本机 Edge 152** - 采集、实现、测试和文档均已完成
  ...
  - **行为层原有 144 项保持一致**，本轮另增 10 项稳定探针覆盖字体解析、TextMetrics 形状和 DOM/Range/Selection；扩探针不必等特定版本
...
- [x] **验证**：1002 项四档全绿；三份 baseline（bootstrap 顺序 344 步 / surface /
...
- **测试**: `npm test`（1002 项 / 110 个文件，Node 18/20/22/24 四档全绿）
