...
- **测试状态**: 884 项（`npm test`，86 个文件）。Node 18 / 20 / 22 / 24
  四档全绿
- **项目性质**: 私有框架，无公开发布计划
...
- [ ] 所有后端异常的句柄泄漏验证
  ...
  - [x] ServiceWorker 注册失败：registration、child Realm 和 ServiceWorker 资源计数均释放
    （`tests/resource-lifecycle-test.js`，child-process / worker-thread）
  - [ ] child-process、worker-thread 的启动异常、协议异常、超时和关闭路径仍需完整覆盖
...
- [x] **验证**：884 项四档全绿；三份 baseline（bootstrap 顺序 344 步 / surface /
...
- **测试**: `npm test`（884 项 / 86 个文件，Node 18/20/22/24 四档全绿）
